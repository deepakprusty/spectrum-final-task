...
#storing as an array in y
y = df['final_grade'].to_numpy()

#storing all columns upto G2 in x as array
col = df.loc[:,"school":"G2"]
x = col.to_numpy()

# to predict and test the accuracy 
def predict(x):
...
#Creating backward elimination model 
...
#Calling the function "predict" to predict the result and the accuracy 
predict(x_modeled)